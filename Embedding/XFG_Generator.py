...
class XFGToPT:
    ...
    def generate_pt_file(self, ll_path, save_dir):
        ...
        for idx in range(len(edge_list)):
            prop_key = str(idx)
            prop_value = branch_properties_data.get(prop_key, 0.0)
        ...
        y_tensor = torch.tensor(dynamic_branch_props, dtype=torch.float).view(-1, 1)  # Ensure y is a column vector
        logging.info(f"Dynamic branch properties shape for {ll_path}: {y_tensor.shape}")
        ...
        if len(dynamic_branch_props) != len(edge_list):
            ...
            y_tensor = torch.tensor(dynamic_branch_props, dtype=torch.float).view(-1, 1)

        # Assign edge attributes and dynamic branch properties
        # Combine edge embeddings and edge type information into a single tensor
        edge_types = torch.tensor(
            [1.0 if graph.edges[edge]["type"] == "data" else 0.0 for edge in graph.edges],
            dtype=torch.float
        ).view(-1, 1)

        data.edge_attr = torch.cat((edge_attr_tensor, edge_types), dim=1)
        data.y = y_tensor
